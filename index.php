...
// Make sure session is started and branch is set
require_once("connection.php");
...
// For security, escape branch for SQL (prepared statements are better, but for consistency with your style, use mysqli_real_escape_string)
$branch_safe = mysqli_real_escape_string($conn, $user_branch);

// Updated table names and columns to match your workflow, filtered by branch

// Total Sales (from sales table)
$total_sales_query = mysqli_query($conn, "SELECT SUM(total_price) AS total_sales FROM sales WHERE branch = '$branch_safe'");
$total_sales = $total_sales_query->fetch_assoc()['total_sales'] ?? 0;

// Total Orders (count of rows in sales table)
$total_orders_query = mysqli_query($conn, "SELECT COUNT(*) AS total_orders FROM sales WHERE branch = '$branch_safe'");
$total_orders = $total_orders_query->fetch_assoc()['total_orders'] ?? 0;

// Total Payrolls
$total_payrolls_query = mysqli_query($conn, "SELECT SUM(gross_pay) AS total_payrolls FROM payrolls WHERE branch = '$branch_safe'");
$total_payrolls = $total_payrolls_query->fetch_assoc()['total_payrolls'] ?? 0;

// Total Staff
$total_staff_query = mysqli_query($conn, "SELECT COUNT(*) AS total_staff FROM staff WHERE branch = '$branch_safe'");
$total_staff = $total_staff_query->fetch_assoc()['total_staff'] ?? 0;

// Total Inventory
$total_inventory_query = mysqli_query($conn, "SELECT COUNT(*) AS total_inventory FROM inventory WHERE branch = '$branch_safe'");
$total_inventory = $total_inventory_query->fetch_assoc()['total_inventory'] ?? 0;

// Total Revenue (from sales)
$total_revenue = $total_sales;

// Fetch recent sales
$recent_sales_res = mysqli_query($conn, "SELECT product_name, quantity_sold, total_price, sale_date FROM sales WHERE branch = '$branch_safe' ORDER BY sale_date DESC LIMIT 5");
$recent_sales = [];
...
$top_products_res = mysqli_query($conn, "SELECT product_name, SUM(quantity_sold) as total_qty FROM sales WHERE branch = '$branch_safe' GROUP BY product_name ORDER BY total_qty DESC LIMIT 5");
$top_products = [];
