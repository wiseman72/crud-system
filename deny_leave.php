...
// Only Admin or Manager for their branch can deny
$username = $_SESSION['username'] ?? '';
$branch = $_SESSION['branch'] ?? '';
$role = $_SESSION['role'] ?? '';

// Confirm role for this branch
...
if (!$leave || $leave['branch'] !== $branch) {
    die("You are not allowed to deny requests from another branch.");
}
...
if ($stmt->execute()) {
    ...
    echo "<p style='color:red;'>Error denying leave: " . $conn->error . "</p>";
}
?>