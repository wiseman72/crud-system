                ...
                if (!$stmt) {
                    ...
                    if ($result && $row = $result->fetch_assoc()) {
                        if (password_verify($password, $row['password'])) {
                            ...
                            $insert_audit = $conn->prepare("INSERT INTO audit_logs (order_id, action, changed_by, old_data, new_data) VALUES (?, ?, ?, ?, ?)");
                            $insert_audit->bind_param("issss", $order_id, $action, $username, $old_data, $new_data);
...
<div class="login-container">
    <form class="login-form" action="login.php" method="post" autocomplete="off">
        ...
        <?php if (!empty($errorMsg)) { echo '<div class="error-message">' . htmlspecialchars($errorMsg) . '</div>'; } ?>
